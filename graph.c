...
/* 1.- Implemente la función createGraph Esta función debe crear y retornar un nuevo grafo vacío. 
Debe reservar memoria para la estructura Graph y luego inicializar su mapa interno (adjacencyMap) utilizando la función constructora del mapa */
...
Si no existe, debe crear una copia del string label (usando strdup o malloc + strcpy), crear una nueva lista vacía 
(que almacenará sus futuras aristas) e insertar el par (copia_label, nueva_lista) en el mapa.*/
...
/* 4.- Implemente la función getEdges(Graph* g, const char* label) Busca un nodo en el mapa a partir de su label
y retorna un puntero a la Lista de aristas (Edge*) que salen de él. Si el nodo no existe en el grafo, debe retornar NULL. */
...
/* 6.- Implemente la función getAdjacentLabels(Graph* g, const char* label) Retorna una nueva 
lista que contenga únicamente los nombres (los strings) de los nodos adyacentes al nodo label.
